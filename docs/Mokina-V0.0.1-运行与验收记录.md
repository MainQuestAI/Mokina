...
| 层级 | 结果 | 证据 |
| --- | --- | --- |
| 源码 | 最新改动后 `pnpm guard`、整仓 `pnpm typecheck`、Web 生产构建通过；相关 daemon 测试 217/217、Web 定向测试 25/25 | 本工作树命令结果；为兼容原上游 UI 测试，Vitest 显式设置原版界面模式，Mokina 默认模式由真实浏览器与桌面检查 |
...
- 历史图片资源冻结、长成果的浏览器交互与 PDF 排版、桌面取消/切换/进程重启恢复。干净目录的安装和启动已通过，完整主链仍由原独立数据根验证；当前自包含版本的 PDF 已验证，依赖外部资源的历史版本按 422 拒绝。
