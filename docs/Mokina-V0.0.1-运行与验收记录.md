...
本轮最终定向执行：daemon 相关测试 26/26，恢复路由关键测试 2/2；营销模式开启的 Web 四组测试 218 通过、3 跳过，原版模式的版本与导出测试 107/107。`pnpm guard`、整仓 `pnpm typecheck`、营销模式 Web 生产构建及 `git diff --check` 均通过。额外全量 Web 测试在 edition-off 配置下为 12,650 通过、9 失败、1 预期失败、11 跳过；失败集中在首页营销入口与上游固定入口断言、zh-CN 占位符、默认遥测配置及一处字重规则，未涉及本轮四项回归，因此不把全量 Web 测试表述为通过。全量 daemon 版本路由文件中另有两例版本写入失败模拟的返回值不符，本轮恢复相关用例通过。
