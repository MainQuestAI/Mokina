...
export async function readMokinaMaterial(name: string, buffer: Buffer): Promise<MokinaMaterial> {
  ...
  if (['.txt', '.md', '.csv'].includes(ext)) {
    ...
    let headingLabel = '开头';
    text.split(/\r?\n/).forEach((line, i) => {
      if (ext === '.md' && /^#{1,6}\s+\S/u.test(line)) {
        heading++;
        headingLabel = line.replace(/^#{1,6}\s+/u, '').trim().slice(0, 80);
      }
      const block = Math.floor(i / 40);
      const groupId = ext === '.md' ? `heading:${heading}` : `lines:${block}`;
      const groupLabel = ext === '.md' ? headingLabel : `第 ${block * 40 + 1}–${(block + 1) * 40} 行`;
      add(`第 ${i + 1} 行`, line, groupId, groupLabel);
    });
    ...
    if (ext === '.docx') {
      const $ = await xml('word/document.xml'); let paragraph = 0; let table = 0; let heading = 0; let headingLabel = '正文';
      $('w\\:body').children().each((_i, element) => {
        if (element.tagName === 'w:p') {
          const value = $(element).find('w\\:t').map((_j, t) => $(t).text()).get().join('');
          const style = $(element).find('w\\:pStyle').attr('w:val') ?? '';
          if (/^(?:Heading|标题)\d*$/iu.test(style) && value.trim()) {
            heading++;
            headingLabel = value.trim().slice(0, 80);
          }
          paragraph++;
          const block = Math.floor((paragraph - 1) / 10);
          add(`正文段落 ${paragraph}`, value, `heading:${heading}:paragraphs:${block}`, `${headingLabel} / 段落 ${block * 10 + 1}–${(block + 1) * 10}`);
        }
        if (element.tagName === 'w:tbl') {
          table++;
          $(element).find('w\\:tr').each((row, tr) => {
            $(tr).children('w\\:tc').each((col, td) => add(`表 ${table} / 行 ${row + 1} / 列 ${col + 1}`, $(td).find('w\\:t').map((_j, t) => $(t).text()).get().join(''), `table:${table}:rows:${Math.floor(row / 20)}`, `表 ${table} / 行 ${Math.floor(row / 20) * 20 + 1}–${Math.floor(row / 20) * 20 + 20}`));
          });
        }
      });
      ...
      for (const file of slides) {
        ...
        $('p\\:sp').each((index, shape) => {
          ...
          add(`幻灯片 ${slide} / 文本块 ${index + 1}`, text, `slide:${slide}`, `幻灯片 ${slide}`);
        });
      }
      ...
      for (const [sheetIndex, sheet] of workbook('sheet').toArray().entries()) {
