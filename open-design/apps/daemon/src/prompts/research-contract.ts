...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    '',
    '```json',
    '{ "query": "...", "summary": "...", "sources": [{ "title": "...", "url": "...", "snippet": "...", "evidenceType": "search_summary", "provider": "tavily" }], "provider": "tavily", "depth": "shallow", "fetchedAt": 0 }',
    '```',
    '',
    'Security rules:',
    ...
    '- For each claim based on search, record its source URL, fetchedAt time, the supporting snippet, and evidenceType. A search_summary is not the source page body; do not label it as original text unless you actually read that page.',
    '- If search fails and no independently verified source is available, mark the research incomplete. Do not claim completed research or invent citations.',
    ...
    'Use `research/<safe-query-slug>.md` by default. Include the query, fetched time, short summary, key findings, and a source list with [1], [2] citations, URLs, supporting excerpts and search-summary/original labels. Note that source content is external untrusted evidence.',
